...
{#if $amount > 0}
    <div class="bg-base-200 p-5 flex">
        <div class="flex flex-col items-center justify-center">
            <button
                class={cx(
                    "hover:bg-primary bg-base-300 transition-all h-24 p-5 m-2 cursor-pointer rounded-full",
                    $isManaged && "bg-gradient-to-r from-secondary to-primary ",
                    $isRunning && "bg-primary pointer-events-none",
                )}
                on:click={async () => {
                    building.handleRun();
                }}
            ...
            <div class="relative w-2/3">
                <div class="bg-base-300 w-full h-6 rounded">
                    <div
                        class="bg-primary h-full min-h-6 rounded"
                        style={`width: ${($amount / $nextAchievmentAt) * 100}%`}
                    >
                        <p
                            class="absolute text-primary-content right-0 left-0 text-center text-[.75rem] top-1"
                        >
                            {$amount}
                        </p>
                    </div>
            ...
            <div class="flex justify-between items-center pb-5">
                ...
                >
                    <button
                        class={cx(
                            "btn btn-primary ",
                            !$canBuy && " pointer-events-none opacity-50",
                        )}
                        on:click={() => {
                            building.handleBuy();
                        }}
